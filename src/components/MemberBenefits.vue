        ...
        <thead>
          ...
          <td v-for="(item, index) in items" :key="index" class="text-center w-1/4">
            <div class="flex flex-col items-center">
              ...
              <span class="text-sm border px-2 py-1 rounded-5 w-23 mt-2 mb-4 bg-gray-200 text-dark-100">{{ item.price ?
                `￥${item.price}/年` : '免费' }}</span>
            </div>
          </td>
        </thead>
        <tbody>
          <tr v-for="(item, index) in allRights" :key="index" :class="index % 2 === 0 ? 'bg-sky-100' : ''">
            <!-- 权限最多的用户渲染的分类 -->
            <td class="text-dark-300">{{ item }}</td>
            <!-- v-for 不同用户不同的该分类的value -->
            <td v-for="(user, idx) in items" :key="idx" class="text-center">
              <div v-if="typeof getValue(item, user) === 'undefined'" class="i-teenyicons:x-circle-solid text-gray-400">
              </div>
              <div v-else-if="
                typeof getValue(item, user) === 'boolean' && getValue(item, user) === true
              " class="i-octicon:check-circle-fill-24 text-sky-500"></div>
